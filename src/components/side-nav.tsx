...
export function SideNav({ activeTab, setActiveTab, isOpen, toggleSidebar }: any) {
    ...
    <>
      ...
      >
        <div className="flex h-16 items-center justify-between px-4 py-4">
          <h1 className={cn("font-bold text-xl transition-opacity", isOpen ? "opacity-100" : "opacity-0 md:hidden")}>
            WhoseBday
          </h1>
          <Button
            variant="ghost"
        ...
        <nav className="flex-1 space-y-2 px-2 py-4">
          {navItems.map((item) => (
            <Button
              ...
              className={cn("w-full justify-start", isOpen ? "px-4" : "px-0 md:justify-center")}
            >
        ...
        <div className="border-t p-4">
          <SignOutButton>
            <Button
              variant="ghost"
              className={cn("w-full justify-start text-red-600 hover:text-red-700", isOpen ? "px-2" : "px-0 md:justify-center")}
            >
